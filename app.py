...
def register():
    if request.method == 'POST':
        ...
        SELECT @out AS the_output;
        """
        params = (request.form['regEmail'], request.form['regPassword'], request.form['regFirstname'], request.form['regLastname']
                  , request.form['departmentCode'], request.form['accountType'])
        cursor = conn.cursor()
        cursor.execute(sql, params)
        result = cursor.fetchone()
        if "Success" == result[0]:
